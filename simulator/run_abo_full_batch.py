    ...
    for i, (vnf_chain, vl_chain) in enumerate(slices):
                ...
                for vl in vl_chain:
                    ...
                    if src in new_placed and dst in new_placed and (src, dst) not in new_routed:
                        ...
                        try:
                            ...
                            g_cost += vl["bandwidth"] + path_latency
        ...
        if result is not None:
            ...
            for (src, dst), path in result.routed_vls.items():
                vl_bw = next(vl["bandwidth"] for vl in vl_chain if vl["from"] == src and vl["to"] == dst)
                for u, v in zip(path[:-1], path[1:]):
                    if (u, v) in link_capacity_global:
                        link_capacity_global[(u, v)] -= vl_bw
                    elif (v, u) in link_capacity_global:
                        link_capacity_global[(v, u)] -= vl_bw
