...
def set_entry_for_slices(slices, entry_node):
    # All comments in English
    return [set_entry_for_slice(sl, entry_node) for sl in slices]


def choose_entry_node(G, seed):
    # All comments in English
...
def main():
    ...
    for num_vnfs in param_grid["num_vnfs_per_slice"]:
        ...
        for seed in param_grid["seed"]:
            entry_node = choose_entry_node(G, seed)

            print(
                f"\n[INFO] === Seed={seed} | selected entry node={entry_node} "
            ...
            for num_slices in param_grid["num_slices"]:
                ...
                print(
                    ...
                    f"{num_vnfs} VNFs (total={total_vnfs}), seed={seed}, entry={entry_node} ==="
                )

                slices_fixed = deepcopy(base_slice_pool[:num_slices])
                slices = set_entry_for_slices(slices_fixed, entry_node)

                print(f"[DEBUG] seed={seed} | entry_node={entry_node}")
                print(f"[DEBUG] first slice sample: {slices[0]}")

                for idx, sl in enumerate(slices[:3]):
                    if isinstance(sl, dict):
                        print(f"[DEBUG] slice {idx} entry={sl.get('entry')}")
                    elif isinstance(sl, (tuple, list)) and len(sl) >= 3:
                        print(f"[DEBUG] slice {idx} entry={sl[2]}")
                ...
                if isinstance(slices[0], dict):
                    ...
                    for k in ["vnfs", "vnf_list", "functions", "chain", "vnf_cpu", "cpu_demands", "vnf_demands"]:
                        if k in slices[0]:
                            ...
                            print(f"[DEBUG] slices[0]['{k}'] sample:", str(slices[0][k])[:500])
